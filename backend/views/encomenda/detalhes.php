...
<div class="encomenda-detalhes">
    ...
    if($models_itemcompra != null){?>
        ...
        <div class="text-center detalhes-btn">
            <?php
            if($model_encomenda->estado != 'cancelado' && $model_encomenda->estado != 'entregue'){
                echo Html::a('Atualizar', ['encomenda/update', 'id_encomenda' => $model_encomenda->id], ['class'=>'btn btn-atualizar']);
            }
            else{
                echo Html::a('Atualizar', ['#'], ['class'=>'btn btn-atualizar disabled']);
            }?>
            <?php
            if($model_encomenda->estado == 'em processamento' || $model_encomenda->estado == 'pendente'){
                echo Html::a('Cancelar', ['encomenda/cancelar', 'id_encomenda' => $model_encomenda->id], ['class'=>'btn btn-cancelar btn-danger']);
            }
            else{
                echo Html::a('Cancelar', ['#'], ['class'=>'btn btn-cancelar btn-danger disabled']);
            }?>
        </div>
    <?php }
    else { ?>
